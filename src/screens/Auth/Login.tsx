...
export const Login = () => {
  ...
  const handleLogin = async () => {
    if (!email || !password) {
      return triggerAlert("Missing Fields", "Please enter both email and password.", "warning");
    }
    ...
    try {
      ...
      if (data?.step === "OTP_REQUIRED" && data.userId) {
        ...
        setStep("OTP");
        setResendTimer(30);
        triggerAlert("OTP Sent", "A verification code has been sent to your registered email.", "success");
      } else {
        triggerAlert("Login Failed", data.message || "Invalid credentials.", "error");
      }
    } catch (error: any) {
      triggerAlert("Network Error", "Invalid credentials or network error. Please try again.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async () => {
    if (!userId) {
      triggerAlert("Session Expired", "Please login again.", "warning");
      setStep("LOGIN");
      return;
    }
    if (!otp || otp.length !== 6) {
      return triggerAlert("Invalid OTP", "Please enter a valid 6-digit OTP.", "warning");
    }
    ...
    try {
      ...
      if (!data.success) {
        return triggerAlert("Verification Failed", data.message || "Invalid OTP.", "error");
      }
      ...
      // Use the global login function! This sets AsyncStorage AND triggers the AuthGuard automatically.
      await login(authUser);

      // Show a nice success popup, then it will auto-navigate due to AuthGuard
      triggerAlert("Welcome Back! 🎉", "Login successful!", "success", () => {
        hideAlert();
      });

    } catch (error: any) {
      triggerAlert("Error", "OTP verification failed.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(ENDPOINTS.RESEND_LOGIN_OTP, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        triggerAlert("OTP Resent", data.message, "success");
        setResendTimer(30);
      }
    } catch (error: any) {
      triggerAlert("Error", "Failed to resend OTP.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Give UI thread a moment to settle
      ...
      triggerAlert("Welcome! 🎉", "Google Login Successful!", "success", () => {
        hideAlert();
      });
      ...
      if (error.code === statusCodes.SIGN_IN_CANCELLED) {
        ...
        triggerAlert("Error", "Play services not available or outdated", "error");
      } else {
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
            ...
            <View style={styles.header}>
              ...
              <Text style={[styles.title, { color: theme.colors.textMain }]}>{step === "LOGIN" ? "Welcome Back" : "OTP Verification"}</Text>
            ...
            {step === "LOGIN" ? (
              <>
                ...
                >
                  ...
                  <Text style={[styles.googleBtnText, { color: theme.colors.textMain }]}>Continue with Google</Text>
                </TouchableOpacity>
              ...
              <>
                <View style={styles.formGroup}>
                  <Text style={[styles.label, { color: theme.colors.textMain }]}>One-Time Password</Text>
                  <View style={[styles.inputWrapper, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}>
                ...
                <View style={styles.timerContainer}>
                  {resendTimer > 0 ? (
                    <Text style={[styles.timerText, { color: theme.colors.textMuted }]}>
                      Resend available in <Text style={[styles.timerHighlight, { color: theme.colors.textMain }]}>{resendTimer}s</Text>
                    </Text>
                  ) : (
                ...
                <TouchableOpacity
                  style={[styles.resendBtn, { backgroundColor: isDarkMode ? theme.colors.border : '#EEF2FF' }, (resendTimer > 0 || loading) && styles.resendBtnDisabled]}
                  onPress={handleResendOtp}
                  disabled={resendTimer > 0 || loading}
                  activeOpacity={0.8}
                >
                  <RefreshCcw color={resendTimer > 0 ? theme.colors.textLight : theme.colors.primary} size={16} />
                  <Text style={[styles.resendBtnText, { color: theme.colors.primary }, resendTimer > 0 && styles.resendBtnTextDisabled]}>
                    {resendTimer > 0 ? `Resend OTP in ${resendTimer}s` : "Resend OTP"}
                  </Text>
                </TouchableOpacity>

                <View style={styles.wrongEmailContainer}>
                  <Text style={[styles.footerText, { color: theme.colors.textMuted }]}>Wrong email? </Text>
                  <TouchableOpacity onPress={() => {
                    setStep("LOGIN");
                    setUserId(null);
                  }}>
                    <Text style={[styles.loginText, { color: theme.colors.primary }]}>Go Back</Text>
                  </TouchableOpacity>
                </View>
              </>
            )}
            ...
            {step === "LOGIN" && (
              <View style={styles.footer}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate("Register")}>
                  <Text style={[styles.loginText, { color: theme.colors.primary }]}>Register Now</Text>
                </TouchableOpacity>
              </View>
            )}
